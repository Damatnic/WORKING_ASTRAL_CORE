...
const nextConfig = {
  ...
  experimental: {
    ...
    // External packages for server components (moved here - correct location)
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    config.resolve = {
      ...
      // Windows-specific resolution improvements (fixed require.resolve usage)
    ...
    if (process.platform !== 'win32') {
      // Fix for ESM context - use import.meta.url instead of __filename
